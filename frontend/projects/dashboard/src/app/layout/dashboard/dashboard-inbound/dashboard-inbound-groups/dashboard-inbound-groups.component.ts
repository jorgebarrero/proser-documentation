...
// Local shared
import { CallsInboundGroupModel } from "projects/dashboard/src/app/shared/models";
import { DashboardInboundGroupsService } from "projects/dashboard/src/app/shared/services";

@Component({
  selector: "app-dashboard-dashboard-inbound-groups",
  templateUrl: "./dashboard-inbound-groups.component.html",
  styleUrls: ["./dashboard-inbound-groups.component.scss"]
})
export class DashboardInboundGroupsComponent implements OnInit {
  ...
  constructor(
    ...
    this.env = envService;
    this.model = new CallsInboundGroupModel();
  ...
  getReportList(userSelection) {
    if (userSelection) {
      this.rows = [new CallsInboundGroupModel()];
      this.dashboardInboundGroupsService.getReportList(userSelection).subscribe(
        res => {
          this.show = false;
