...
export class CrudInvSupervisorComponent implements OnInit, OnDestroy {
  ...
  onNewRecord($event?) {
    ...
    localStorage.setItem(
      "recordSelection",
      JSON.stringify([new InvSupervisorModel()])
    );
  }
  ...
  // BUTTON FOR ACTIVE RECORDS ONLY
  onGetActive($event?) {
    this.show_datatable = true;
    this.show_selected_button = false;
    this.selection = new InvSupervisorModel();
  ...
  // BUTTON FOR INACTIVE RECORDS ONLY
  onGetInactive($event?) {
    this.show_datatable = true;
    this.show_selected_button = false;
    this.selection = new InvSupervisorModel();
