...
@Component({
  selector: 'app-crud-inv-agent-role',
  templateUrl: './crud-inv-agent-role.component.html',
  styleUrls: ['./crud-inv-agent-role.component.scss']
})
export class CrudInvAgentRoleComponent implements OnInit {
  ...
  selection: InvAgentRoleModel;
  action: ActionConfig;

  rows;
  rows_original;
  selected: [{ selected: InvAgentRoleModel }];
  query;
  ...
  constructor(
    ...
    this.status_field = 'inv_agentrole_status';
    ...
    this.selectedFilterField = {field_name: 'inv_agentrole_name', name: 'nombre', text: 'nombre' };
  ...
  getAll_Records(query?) {
    this.InvAgentRoleService.getAllRecords(query).subscribe(
      (data: InvAgentRoleModel[]) => {
    ...
    getSingle_Record(event?) {
      ...
      let id = event.temp.inv_agentrole_id

      this.InvAgentRoleService.getRecordById(id).subscribe(
        (temp) => {
          let data = [temp]
          this.rows_original = data;
          this.rows = data;
  ...
  onNewRecord($event?) {
    ...
    this.action.temp = new InvAgentRoleModel();
  }
  ...
  onExportToExcel(data, name) {
    ...
    const filterData = data.map(x => {
      return {
        id: x.inv_agentrole_id,
        nombre: x.inv_agentrole_name,
        estatus: x.inv_agentrole_status
      };
    });
