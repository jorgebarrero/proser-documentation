...
@Component({
  selector: 'app-crud-inv-client',
  templateUrl: './crud-inv-client.component.html',
  styleUrls: ['./crud-inv-client.component.scss']
})
export class CrudInvClientComponent implements OnInit {
  ...
  selection: InvClientModel;
  action: ActionConfig;

  rows;
  rows_original;
  selected: [{ selected: InvClientModel }];
  query;
  ...
  constructor(
    ...
    this.status_field = 'inv_client_status';
    this.excel_subtitle= 'clientes';
    ...
    this.selectedFilterField = {field_name: 'inv_client_name', name: 'nombre', text: 'nombre' };
  ...
  getAll_Records(query?) {
    this.InvClientService.getAllRecords(query).subscribe(
      (data: InvClientModel[]) => {
    ...
    getSingle_Record(event?) {
      ...
      let id = event.temp.inv_client_id

      this.InvClientService.getRecordById(id).subscribe(
        (temp) => {
          let data = [temp]
          this.rows_original = data;
          this.rows = data;
  ...
  onNewRecord($event?) {
    ...
    this.action.temp = new InvClientModel();
  }
  ...
  onExportToExcel(data, name) {
    ...
    const filterData = data.map(x => {
      return {
        id: x.inv_client_id,
        nombre_cliente: x.inv_client_name,
        nombre_corto: x.inv_client_shortname,
        tipo_cliente: x.inv_client_type,
        estatus: x.inv_client_status,
