...
@Component({
  selector: 'app-crud-inv-auxiliar',
  templateUrl: './crud-inv-auxiliar.component.html',
  styleUrls: ['./crud-inv-auxiliar.component.scss']
})
export class CrudInvAuxiliarComponent implements OnInit {
  ...
  selection: InvAuxiliarModel;
  action: ActionConfig;

  rows;
  rows_original;
  selected: [{ selected: InvAuxiliarModel }];
  query;
  ...
  getAll_Records(query?) {
    this.invAuxiliarService.getAllRecords(query).subscribe(
      (data: InvAuxiliarModel[]) => {
    ...
    getSingle_Record(event?) {
      ...
      this.invAuxiliarService.getRecordById(id).subscribe(
        (temp) => {
          let data = [temp]
          this.rows_original = data;
          this.rows = data;
  ...
  onGetActive($event?) {
    ...
    this.show_selected_button = false;
    this.selection = new InvAuxiliarModel();
    this.query = `{"where":{"${this.status_field}":"A" , "${this.productivity_field}":"0"}}`;
  ...
  onGetInactive($event?) {
    ...
    this.show_selected_button = false;
    this.selection = new InvAuxiliarModel();
    this.query = `{"where":{"${this.status_field}":"I" , "${this.productivity_field}":"0"}}`;
    this.getAll_Records(this.query);
  }
  ...
  onGetAll($event?) {
    ...
    this.show_selected_button = false;
    this.selection = new InvAuxiliarModel();
    this.query = `{"where":{"${this.productivity_field}":"0"}}`
    this.getAll_Records(this.query);
  }
  ...
  onNewRecord($event?) {
    ...
    this.action.temp = new InvAuxiliarModel();
  }
