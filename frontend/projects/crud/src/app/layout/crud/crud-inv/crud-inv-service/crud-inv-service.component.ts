...
@Component({
  selector: 'app-crud-inv-service',
  templateUrl: './crud-inv-service.component.html',
  styleUrls: ['./crud-inv-service.component.scss']
})
export class CrudInvServiceComponent implements OnInit {
  ...
  selection: InvServiceModel;
  action: ActionConfig;

  rows;
  rows_original;
  selected: [{ selected: InvServiceModel }];
  query;
  ...
  constructor(
    ...
    this.selectedFilterField = {field_name: 'inv_service_name', name: 'nombre', text: 'nombre' };
  ...
  getAll_Records(query?) {
    this.invServiceService.getAllRecords(query).subscribe(
      (data: InvServiceModel[]) => {
    ...
    getSingle_Record(event?) {
      ...
      this.invServiceService.getRecordById(id).subscribe(
        (temp) => {
          let data = [temp]
          this.rows_original = data;
          this.rows = data;
  ...
  onNewRecord($event?) {
    ...
    this.action.temp = new InvServiceModel();
  }
