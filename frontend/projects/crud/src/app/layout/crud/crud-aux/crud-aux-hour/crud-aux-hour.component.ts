...
@Component({
  selector: 'app-crud-aux-hour',
  templateUrl: './crud-aux-hour.component.html',
  styleUrls: ['./crud-aux-hour.component.scss']
})
export class CrudAuxHourComponent implements OnInit {
  ...
  selection: AuxHourModel;
  action: ActionConfig;

  rows;
  rows_original;
  selected: [{ selected: AuxHourModel }];
  query;
  ...
  constructor(
    ...
    this.status_field = 'aux_hour_status';
    this.excel_subtitle= 'horas';
    ...
    this.selectedFilterField = {field_name: 'aux_hour_name', name: 'nombre', text: 'nombre' };
  ...
  getAll_Records(query?) {
    this.auxHourService.getAllRecords(query).subscribe(
      (data: AuxHourModel[]) => {
    ...
    getSingle_Record(event?) {
      ...
      let id = event.temp.aux_hour_id

      this.auxHourService.getRecordById(id).subscribe(
        (temp) => {
          let data = [temp]
          this.rows_original = data;
          this.rows = data;
  ...
  onNewRecord($event?) {
    ...
    this.action.temp = new AuxHourModel();
  }
  ...
  onExportToExcel(data, name) {
    this.show_datatable = true;
    let temp = (this.model.fieldInfo('aux_hour_value').name)
    const filterData = data.map(x => {
      return {
        id : x.aux_hour_id,
        nombre: x.aux_hour_name,
        valor: x.aux_hour_value,
        estatus: x.aux_hour_status
      };
    });
