...
@Component({
  selector: "app-reports-auxiliar-report-list",
  templateUrl: "./auxiliar-report-list.component.html",
  styleUrls: ["./auxiliar-report-list.component.scss"]
})
export class AuxiliarReportListComponent implements OnInit {
  ...
  constructor(
    ...
    this.model = new AgentsAuxiliarModel();
  ...
  ngOnInit() {
    ...
    this.exportNameGroup = "reporte-auxiliar-consolidado";
    this.exportNameDetail = "reporte-auxiliar-detalle";
  ...
  getReportList(userSelection) {
    if (userSelection) {
      this.rows = [new AgentsAuxiliarModel()];
      this.agentsAuxiliarService.getReportList(userSelection).subscribe(
  ...
  onCreateModel(model?) {
    model = new AgentsAuxiliarModel().fieldList();
