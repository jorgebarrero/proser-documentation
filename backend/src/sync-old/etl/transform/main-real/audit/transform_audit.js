...
async function transformRealAudit(start_date) {
      ...
      if (validation) {
        try {
          ...
          written = await writeDestiny(extendedResult, destinyTable).catch(
            err =>
              console.log("transformRealAudit caught it - writeDestiny", err)
          );

          return "transformRealAudit end";
        } catch (e) {
          console.log("e", e);
          return e;
        }
