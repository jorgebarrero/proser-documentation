...
async function transformRealCdrHca(start_date) {
  console.log(`/*************/ Transform transformRealCdrHca /*************/ `);
  ...
  if (start_date) {
    ...
    if (Array.isArray(result) && result.length > 0) {
      ...
      if (validation) {
        ...
        written = await writeDestiny(extendedResult, destinyTable).catch(err =>
          console.log("transformRealCdrHca caught it - writeDestiny", err)
        );
    ...
    return "transformRealCdrHca end";
  }
}
