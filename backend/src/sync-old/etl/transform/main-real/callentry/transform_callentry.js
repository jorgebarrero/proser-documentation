...
const originTable = "RealCallEntry";
...
async function transformRealCallEntry(start_date) {
      ...
      if (validation) {
        try {
          ...
          written = await writeDestiny(extendedResult, destinyTable).catch(
            err =>
              console.log(
                "transformRealCallEntry caught it - writeDestiny",
                err
              )
          );
          return "transformRealCallEntry end";
        } catch (e) {
          console.log("e", e);
          return e;
        }
