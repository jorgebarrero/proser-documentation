...
async function updateRealCallEntry(start_date) {
  let result = null;
  let temp_date;
  ...
  temp_date = await minDate(originTable, originDateField).catch(err =>
    console.log("updateRealCalEntry caught it - minDate", err)
  );
    ...
    if (validation) {
      try {
        ...
        written = await writeDestiny(extendedResult, destinyTable).catch(err =>
          console.log("updateRealCalEntry caught it - writeDestiny", err)
        );

        return "updateRealCalEntry end";
      } catch (e) {
        console.log("e", e);
        return e;
      }
